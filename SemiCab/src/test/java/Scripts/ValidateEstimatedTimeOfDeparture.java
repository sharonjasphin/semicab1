...
public class ValidateEstimatedTimeOfDeparture extends Base{
	@Test(groups = {"regression"} , priority = 1)
	public void ValidateEstimatedDeparture() throws Throwable {
		...
		cu.waitTillElementToBeClickable(driver, l.getEstimatedTimeOfDeparture());
		l.getEstimatedTimeOfDeparture().click();
		...
		String[] vt=l.getVerifyEstimatedTimeOfDeparture().getText().split(",");
		SimpleDateFormat format1=new SimpleDateFormat("MM/dd/yyyy");
		java.util.Date datedt = format1.parse(dt[0]);
		System.out.println(datedt);
		vt[0]=vt[0].replaceAll("th", "").replaceAll("2nd", "02").replaceAll("1st", "01").replaceAll("3rd", "03").replaceAll(" ", "-").replaceAll("Estimated-Time-of-Departure", "");	
		vt[0]=vt[0].replace("\n", "");	

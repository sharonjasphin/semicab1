...
public class ValidateArrivedStatusEvent extends Base{

	@Test(groups = {"regression"} , priority = 1)
	public void ValidateArrivedStatus() throws Throwable {
		...
		String[] vt=l.getVarifyArrivedTime().getText().split(",");
		SimpleDateFormat format1=new SimpleDateFormat("MM/dd/yyyy");
		java.util.Date dateat = format1.parse(at[0]);
		System.out.println(dateat);
		vt[0]=vt[0].replaceAll("th", "").replaceAll("2nd", "02").replaceAll("1st", "01").replaceAll("3rd", "03").replaceAll(" ", "-").replaceAll("Arrived", "");
		vt[0]=vt[0].replace("\n", "");
		...
		System.out.println("Arrival actual start date and time==>"+dateat+" "+timeat);
		System.out.println("Arrival expected start date and time==>"+datevt+" "+timevt);
		test.log(LogStatus.PASS, "Arrival actual start date and time==>"+dateat+" "+timeat);
		test.log(LogStatus.PASS, "Arrival expected start date and time==>"+datevt+" "+timevt);
